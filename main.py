...
while True:
    ...
    if chooser == "semester":
            ...
            if grades_str.lower() != "none":
                ...
                for entry in grade_entries:
                    try:
                        ...
                        for _ in range(num):  # Use _ for unused loop variable
                            if letter in letter_values:
                                points += letter_values[letter]
                                count += 1
                    except (ValueError, IndexError):
                        print("Invalid grade format: ", entry + " Make sure you type in the format Letter(Number) and separate entries with commas.")  # Improved error message
                        return 0, 0
            return points, count

        academic_string = input("Enter semester letter grades of all ACADEMIC (4.0) courses you took or are taking. Use the format Letter(Number) and separate entries with commas. If you're taking none, type 'none' : ")
        kap_ap_string = input("Enter semester letter grades of all KAP and AP (5.0) courses you took or are taking. Use the format Letter(Number) and separate entries with commas. If you're taking none, type 'none' : ")
        ...
        def get_rank(gpa):
            for lower, upper, rank in gpa_ranks:
                if lower <= gpa < upper:
                    return rank
            return "GPA not in defined range" 
